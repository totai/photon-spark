...
//agenda = new (Agenda);
   agenda = new Agenda({db: {address: config.mongo.uri, collection: 'spark_schedule'}},
      function (err) {
         if (err) {
            console.log(err);
            throw err;
...
agenda.on('start', function (job) {
   var today = new Date();
   console.log(today + " : " +JSON.stringify(job) + ' -> starting: ');
});
...
// var T = new Date(later.schedule(sched).prev(1));
